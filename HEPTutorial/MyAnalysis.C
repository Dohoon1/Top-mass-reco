...
void MyAnalysis::SlaveBegin(TTree * /*tree*/) {
   ...
   histograms.push_back(h_minDeltaPhi_MET_Muon);
   histograms_MC.push_back(h_minDeltaPhi_MET_Muon);
   ...
   histograms.push_back(h_minDeltaPhi_MET_BJet);
   histograms_MC.push_back(h_minDeltaPhi_MET_BJet);
...
Bool_t MyAnalysis::Process(Long64_t entry) {
   ...
   // +trigger bit
   //if (triggerIsoMu24 && N_IsoMuon>0 && N_BJet > 0) {
      ...
      if (muon1->Pt()>MuonPtCut) {
         ...
         for (vector<MyJet>::iterator it = Jets.begin(); it != Jets.end(); ++it) {
            ...
            if (it->IsBTagged()) {
               ++N_BJet_tmp;
               if (N_BJet_tmp == 1) {
                  h_BJet1_Pt->Fill(it->Pt(), EventWeight*SF_b);
                  h_BJet1_Eta->Fill(it->Eta(), EventWeight*SF_b);
   ...
   //////////////////////////////
   // 4b) Top mass (hadroninc top decay) (reco)
   // 4c) Top mass (leptonic top decay) (reco)
   if (IsSelected) {
      
      // first, the hadronic branch
      // find combination with one b-jet and two non-b-jets which are compatible with a
      // W boson mass (65<M<95)
      for (vector<MyJet>::iterator it = Jets.begin(); it != Jets.end(); ++it) {
         if (it->IsBTagged()) {
                  ...
                  for (vector<MyJet>::iterator kt = jt; kt != Jets.end(); ++kt) {
                     if (kt != it && kt != jt && !(kt->IsBTagged())) {
                        if (((*jt) + (*kt)).M() > 65 && ((*jt) + (*kt)).M() < 95){
                           ...
                           // now calculate the three-jet mass
      ...
      // now the leptonic branch
      // combine the muon with the missing energy (neutrino)
      // only px and py of the missing energy are available.
      // --->Calculate the z-component from a W-mass constraint (2 solutions)
      for (vector<MyJet>::iterator it = Jets.begin(); it != Jets.end(); ++it) {
         if (it->IsBTagged()) {
            double px = met.Px();
            double py = met.Py();
            double mW = 80.4;
            double A = pow(muon1->E(), 2) - pow(muon1->Pz(), 2);
            double B = mW * mW / 2. + (muon1->Px() * px + muon1->Py() * py);
            double D = pow(muon1->E(), 2) * (pow(B, 2) - pow(met.Pt(), 2) * A);
            // first solution
            if (D >= 0) {
               double pz = muon1->Pz() * B / A + sqrt(D) / A;
               double E = sqrt(px * px + py * py + pz * pz);
               TLorentzVector neutrino1(px, py, pz, E);
               h_Mbln_reco->Fill(((*it) + (*muon1) + neutrino1).M(), EventWeight);
            }
            // second solution
            if (D > 0) {
               double pz = muon1->Pz() * B / A - sqrt(D) / A;
